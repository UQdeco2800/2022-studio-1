...
/**
 * Part of the memento design pattern, originator is the object to store the states of the player, this includes the
 * player's inventory states (e.g. amount of gold, type of sword, items in inventory, health and base attack value
 *
 * Can be extended to save states for other entities in the future or use as checkpoints for player to respawn
 */
public class Originator {
    private static final Logger logger = LoggerFactory.getLogger(Originator.class);
    protected int state, gold, currentHealth, attack;
    protected List<Artefact> items;

    /**
     * Originator constructor which can create a new originator to store the current state of the player before entering
    ...
    /**
     * saves the attack value for the player, this includes the boost for player from different weapons bought in the
     * shop
